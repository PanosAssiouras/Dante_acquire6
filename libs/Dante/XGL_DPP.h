...
/** Used to get the status of the connected systems: check if acquisition is finished and all data have been collected by the DLL
*	Returns true if the "running" parameter is valid.
*	Returns false if an error occours. */
EXTERN DECLSPEC uint32_t   CALL_CONV isTransmissionEnded(const char * identifier, uint16_t Board); // Currently not implemented

/** Disabled board will not generate any data. */
EXTERN DECLSPEC uint32_t   CALL_CONV disableBoard(const char* identifier, uint16_t Board, bool disable); // Currently not implemented


/** To perform a single measure:
* Set time to "0" to get a free-running measure.
* Time is expressed in seconds. */
EXTERN DECLSPEC uint32_t   CALL_CONV start(const char * identifier, float time, const uint16_t spect_depth);
...
/** To perform an acquisition in list wave mode:
* Set time to "0" get a free-running measure.
* Time is expressed in seconds. */
...
// Get how many data there is in the answers buffer.
EXTERN DECLSPEC bool CALL_CONV GetAnswersDataLength(uint32_t * length);

// Get data of the hardware answers.
EXTERN DECLSPEC bool CALL_CONV GetAnswersData(uint32_t length, uint32_t * data);
